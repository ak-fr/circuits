...
def connectors_number_per_side(horizontal_size,vertical_size,horizontal_pitch,vertical_pitch,N):
    ...
    sides_connectors_number = [north_side_pins_number, south_side_pins_number, east_side_pins_number, west_side_pins_number]
    sides_connectors_spacing= [north_spacing,south_spacing,east_spacing,west_spacing]
    #Order: North, South, East, West
    print('############################################',north_side_capacity,'\n',sides_connectors_number,'\n',sides_connectors_spacing)
...
#The inputs are a dictionaary containing signals from vst, sides and pitches sizes
def signals_per_side(dico,horizontal_size,vertical_size,horizontal_pitch,vertical_pitch):
  ...
  bits_left        = 0
  last_bit_placed  = 0
  while north_pin_placed < sides_connectors_number[0]:
          name_of_vector_placed,size_placed_vector,start_b = dico[sorted_indexes_dico[i]]
          bits_placed_number = min(sides_connectors_number[0]-north_pin_placed,dico[sorted_indexes_dico[i]][1])
          ...
          if bits_placed_number == dico[sorted_indexes_dico[i]][1]:
            ...
            #the missing bits will be placed on the other sides starting by the south
            bits_left = dico[sorted_indexes_dico[i]][1]- bits_placed_number

  while south_pin_placed < sides_connectors_number[1]:
          name_of_vector_placed,size_placed_vector,start_b = dico[sorted_indexes_dico[i]]
          if bits_left != 0:
          #place the last North vector missing bits (if they are not all placed)     
           ...
           if bits_placed_number == bits_left:
            #all the bits of the last North vector are placed  
            i=i+1
            bits_left=0
           else:
          ...
          #all bits from vectors in the north are placed, we will place new vectors in this side  
           bits_placed_number = min(sides_connectors_number[1]-south_pin_placed,dico[sorted_indexes_dico[i]][1])
  ...
  while west_pin_placed < sides_connectors_number[3]:
          name_of_vector_placed,size_placed_vector,start_b = dico[sorted_indexes_dico[i]]
          if bits_left != 0:
           bits_placed_number = min(sides_connectors_number[3],bits_left)  
           west.append([name_of_vector_placed ,start_b+last_bit_placed +1 ,start_b+last_bit_placed+ bits_placed_number,vector_or_bit(size_placed_vector)])
           ...
           bits_placed_number = min(sides_connectors_number[3]-west_pin_placed,dico[sorted_indexes_dico[i]][1])
           ...
           if bits_placed_number == dico[sorted_indexes_dico[i]][1]:
            ...
            bits_left = dico[sorted_indexes_dico[i]][1]- bits_placed_number
  return sides_connectors_pitches,[north,south,east,west]
    
  

#generate ioPinsSpec to be written in doDesign.py
def generate_ioPinsSpec(dico,horizontal_size,vertical_size,horizontal_pitch,vertical_pitch):
    spacings,connectors =  signals_per_side(dico,horizontal_size,vertical_size,horizontal_pitch,vertical_pitch)
    ioPinsSpec = 'ioPinsSpec \t=['
    side=['NORTH','SOUTH','EAST','WEST']
    pitch_id=['m2pitch','m2pitch','m1pitch','m1pitch']
    for s in range (4):
     pitch_shift =0   
     ...
     for i in range (len(connectors[s])):
        ...
        if i==0:
           ...
           start =f'(1+{pitch_shift})*{spacings[s]} *{pitch_id[s]}'
        ioPinsSpec = ioPinsSpec+f'({side_name}, "{name}",{start},{spacing_between_bits},{range_pin})'+','+'\n'+'\t'
    ioPinsSpec = ioPinsSpec[:-3]+']'  #to remove the ',' at the end
    return ioPinsSpec   
    
#same function than before but written as a function to be called
def generate_ioPinsSpec_list(dico,horizontal_size,vertical_size,horizontal_pitch,vertical_pitch):
    spacings,connectors =  signals_per_side(dico,horizontal_size,vertical_size,horizontal_pitch,vertical_pitch)
    ioPinsSpec = []
    side=['NORTH','SOUTH','EAST','WEST']
    pitch_id=['m2pitch','m2pitch','m1pitch','m1pitch']
...
#def routine_with_sta(file,entity,h_area,v_area,horizontal_pitch,vertical_pitch,comb,lib,model,clk_name='clk',reset_name='reset'):
# routine(file,entity,h_area,v_area,horizontal_pitch,vertical_pitch,comb,clk_name='clk',reset_name='reset')
# if comb ==  1:
#  entity_to_run = f'{entity}_r'
# elif comb == 0:
#  entity_to_run = f'{entity}_cts_r'
# generate_db_hitas_file(entity_to_run,model)
# generate_report_hitas_file('./hitas/report.tcl',entity_to_run)
# modify_spi_file(f'{entity_to_run}.spi',lib)
# modify_hitas_spi_file('template_hitas.spi',entity_to_run,f'{entity_to_run}.spi')
# cmd = f'ssh aoudrhiri@bip "cd /users/cao/aoudrhiri/coriolis-2.x/src/alliance-check-toolkit/benchs/automated_flow/sky130_c4m; source hitas/avt_env.sh; ./hitas/db_{entity_to_run}.tcl; ./hitas/report.tcl"'
...
#dico = get_signals_hurricane("picorv32")
###dico = get_signals("picorv32.vst","picorv32")
#h,v= get_area_in_units('init_template_doDesign.py')
###print(dico,h,v)
##signals_per_side(dico,h,v,horizontal_pitch,vertical_pitch)
#print(generate_ioPinsSpec(dico,h,v,horizontal_pitch,vertical_pitch))
